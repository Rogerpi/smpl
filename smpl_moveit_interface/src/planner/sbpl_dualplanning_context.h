#ifndef sbpl_interface_SBPLDualPlanningContext_h
#define sbpl_interface_SBPLDualPlanningContext_h
...
class SBPLDualPlanningContext : public planning_interface::PlanningContext
{
public:
    ...
    MoveItRobotModel* m_robot_model_dual;
    MoveItRobotModel* m_robot_model_first;
    std::unique_ptr<MoveItCollisionChecker> m_collision_checker_dual;
    std::unique_ptr<MoveItCollisionChecker> m_collision_checker_first;

    std::unique_ptr<smpl::OccupancyGrid> m_grid;

    std::unique_ptr<smpl::DualPlannerInterface> m_planner;
...
MOVEIT_CLASS_FORWARD(SBPLDualPlanningContext);
