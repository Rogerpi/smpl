...
auto UpdateOrCreateGrid(
    SBPLDualPlanningContext* context,
    ...
    -> std::unique_ptr<smpl::OccupancyGrid>
{
    ...
    if (scene != context->m_prev_scene || workspace_diff) {
        ...
        return CreateHeuristicGrid(
                *scene,
                workspace,
                context->m_robot_model_dual->planningGroupName(),
    ...
    const moveit_msgs::Constraints& constraints)
{
    ...
    context->m_collision_checker_dual = smpl::make_unique<MoveItCollisionChecker>();
    if (!context->m_collision_checker_dual->init(context->m_robot_model_dual, start_state, scene)) {
        ROS_WARN_NAMED(PP_LOGGER, "Failed to initialize dual collision checker interface");
        return false;
    }
    context->m_collision_checker_first = smpl::make_unique<MoveItCollisionChecker>();
    if (!context->m_collision_checker_first->init(context->m_robot_model_first, start_state, scene)) {
        ROS_WARN_NAMED(PP_LOGGER, "Failed to initialize dual collision checker interface");
        return false;
    }
    //TODO dual: What are constraints?
    context->m_collision_checker_dual->setPathConstraints(constraints);
    context->m_collision_checker_first->setPathConstraints(constraints);
    ...
    context->m_planner = smpl::make_unique<smpl::DualPlannerInterface>(
            context->m_robot_model_dual, context->m_collision_checker_dual.get(),
            context->m_robot_model_first, context->m_collision_checker_first.get(),
            context->m_grid.get());
...
SBPLDualPlanningContext::SBPLDualPlanningContext(
    MoveItRobotModel* robot_model_dual,
    MoveItRobotModel* robot_model_first,
    const std::string& name,
    const std::string& group)
:
    Base(name, group),
    m_robot_model_dual(robot_model_dual),
    m_robot_model_first(robot_model_first),
    m_collision_checker_dual(),
    m_collision_checker_first(),
    m_grid(),
    m_planner()
{
...
bool SBPLDualPlanningContext::solve(planning_interface::MotionPlanResponse& res)
{
...
bool SBPLDualPlanningContext::solve(
    planning_interface::MotionPlanDetailedResponse& res)
{
    ROS_INFO_NAMED(PP_LOGGER, "SBPLDualPlanningContext::solve(planning_interface::MotionPlanDetailedResponse&)");
...
bool SBPLDualPlanningContext::terminate()
{
    ROS_INFO_NAMED(PP_LOGGER, "SBPLDualPlanningContext::terminate()");
    return true;
}

void SBPLDualPlanningContext::clear()
{
    ROS_INFO_NAMED(PP_LOGGER, "SBPLDualPlanningContext::clear()");
}

// Initialize an SBPLDualPlanningContext. Checks for parameters required by the
// context, prepares parameters for PlannerInterface initialization
bool SBPLDualPlanningContext::init(const std::map<std::string, std::string>& config)
{
    ...
    if (!m_robot_model_dual->initialized()) {
        ROS_ERROR_NAMED(PP_LOGGER, "MoveIt! Dual Robot Model is not initialized");
        return false;
    }
    if (!m_robot_model_first->initialized()) {
        ROS_ERROR_NAMED(PP_LOGGER, "MoveIt! First Robot Model is not initialized");
        return false;
    }
    ROS_INFO("PARAMS:");

    for( auto& c : config){
        ROS_INFO_STREAM("First: " << c.first << " Second: " << c.second);
    }

    //TODO dual: Correct params for each model. Will be hardcoded later for now probably
    ...
    m_use_grid =   (heuristic_name == "bfs" || 
                    heuristic_name == "mfbfs" || 
                    heuristic_name == "bfs_egraph" || 
                    heuristic_name == "mbfs" ||
                    heuristic_name == "bfs_rpy");
