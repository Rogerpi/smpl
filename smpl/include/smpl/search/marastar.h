...
// project includes
#include <smpl/heap/multi_index_intrusive_heap.h>
#include <smpl/time.h>
...
private:
    ...
    {
        if(current_planning_group==GroupType::BASE)
        {   
    ...
    void expand(SearchState* s, GroupType group);

    void recomputeHeuristics();
    void reorderOpen();
    int computeKey(SearchState* s, GroupType planning_group);
