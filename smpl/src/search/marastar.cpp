    ...
    int* cost)
{
    ...
    if (m_start_state_id != m_last_start_state_id) {
        ...
        m_open_arm.clear(GroupType::BASE);
        m_open_base.clear(GroupType::ARM);
        m_open_base_iso.clear(GroupType::BASE_ISO);
        m_incons.clear();
        ...
        start_state->g = 0;
        start_state->f[0] = computeKey(start_state,GroupType::BASE);
        start_state->f[1] = computeKey(start_state,GroupType::ARM);
        m_open_base.push(start_state,GroupType::BASE);
        m_open_base_iso.push(start_state,GroupType::BASE_ISO);
        m_open_arm.push(start_state,GroupType::ARM);
    ...
    while (m_satisfied_eps > m_final_eps) {
        ...
        if (m_curr_eps == m_satisfied_eps) {
            ...
            for (SearchState* s : m_incons) {
                ...
                s->incons = false;
                m_open_base.push(s,GroupType::BASE);
                m_open_base_iso.push(s,GroupType::BASE_ISO);
                m_open_arm.push(s,GroupType::ARM);
            }
...
int MARAStar::force_planning_from_scratch_and_free_memory()
{
    ...
    m_open_base.clear(GroupType::BASE);
    m_open_base_iso.clear(GroupType::BASE_ISO);
    m_open_arm.clear(GroupType::ARM);
...
void MARAStar::recomputeHeuristics()
{
    for (auto s : m_states) {
        if (s != NULL) {
        s->h[0] = m_heur->GetGoalHeuristic(s->state_id, GroupType::BASE);
        s->h[1] = m_heur->GetGoalHeuristic(s->state_id, GroupType::ARM);
    ...
    clock::duration& elapsed_time)
{
    ...
    while (!empty) {
            ...
            if(!m_open_base_iso.empty())
            {
                ...
                m_open_base_iso.pop(GroupType::BASE_ISO);
                

                assert(min_state->iteration_closed[GroupType::BASE] != m_iteration);
                assert(min_state->g != INFINITECOST);
                min_state->iteration_closed[GroupType::BASE] = m_iteration;

                assert(min_state->iteration_closed[GroupType::ARM] != m_iteration);
                assert(min_state->g != INFINITECOST);
                min_state->iteration_closed[GroupType::ARM] = m_iteration;

                min_state->eg = min_state->g;

                expand(min_state, GroupType(GroupType::BASE_ISO));
            ...
            if(!m_open_base.empty())
            {
                ...
                m_open_base.pop(GroupType::BASE);
                ...
                expand(min_state, GroupType(GroupType::BASE));
            ...
            if(!m_open_arm.empty())
            {
                ...
                expand(min_state, GroupType(GroupType::ARM));
...
{   
    ...
    SMPL_DEBUG_STREAM("Min Heuristic found is "<<current_min<<" and base min before "
        <<min_heuristic_found[GroupType::BASE]<<" arm min before"
        <<min_heuristic_found[GroupType::ARM]);

    if(current_min<min_heuristic_found[current_planning_group])// && current_min<min_heuristic_found[GroupType::ARM])
    {
        min_heuristic_found[current_planning_group] = current_min;
        SMPL_DEBUG_STREAM("Keep in the same group "<<current_planning_group<<" and new min "<<min_heuristic_found[current_planning_group]);
        return;
    }
    else if(min_heuristic_found[GroupType::BASE]!=-1 && min_heuristic_found[GroupType::ARM]!=-1 )
    {
            if(current_planning_group == GroupType::BASE)// && min_heuristic_found[GroupType::ARM]<=min_heuristic_found[GroupType::BASE])
            {
                //SMPL_DEBUG_STREAM("Switching to group arm!");
                current_planning_group = GroupType::ARM;
                expansion_status[GroupType::BASE] = 1;
            }
            else if(current_planning_group == GroupType::ARM)// && min_heuristic_found[GroupType::BASE]<=min_heuristic_found[GroupType::ARM])
            {
                //SMPL_DEBUG_STREAM("Switching to group base!");
                current_planning_group = GroupType::BASE;
                expansion_status[GroupType::ARM] = 1;
            }
    }
    //min_heuristic_found[GroupType::BASE]<=min_heuristic_found[GroupType::ARM])
*/
}

void MARAStar::synchronizeGroupsOpenLists()
{
   /* if (expansion_status[GroupType::BASE] && expansion_status[GroupType::ARM])
    {
        expansion_status[GroupType::BASE] = 0;
        expansion_status[GroupType::ARM] = 0;
        for(int i=0;i<m_states.size();i++)
        {
            if(m_open.contains(m_states[i],GroupType::BASE) 
                && m_open.contains(m_states[i],GroupType::ARM))
...
void MARAStar::expand(SearchState* s, GroupType group)
{
    m_succs.clear();
    m_costs.clear();
    ...
    if(group==GroupType::BASE_ISO)
    {
        m_space->GetSuccsByGroup(s->state_id, &m_succs, &m_costs, GroupType::BASE); 
        ...
        for (size_t sidx = 0; sidx < m_succs.size(); ++sidx) {
            ...
            if (new_cost < succ_state->g) {
                ...
                if (succ_state->iteration_closed[GroupType::BASE] != m_iteration) {
                    succ_state->f[0] = computeKey(succ_state,GroupType::BASE);
                    
                    SMPL_DEBUG_STREAM("State "<<succ_state->state_id<< " has base_iso f-val "<<succ_state->f[0]);
                    if (m_open_base_iso.contains(succ_state,GroupType::BASE_ISO)) {
                        SMPL_DEBUG_STREAM("State "<<succ_state->state_id<< " has already been expanded in base decrease priority!");
                        m_open_base_iso.decrease(succ_state,GroupType::BASE_ISO);
                    }
                    else {
                        m_open_base_iso.push(succ_state,GroupType::BASE_ISO);
                    ...
                    if (m_open_base.contains(succ_state,GroupType::BASE)) {
                        SMPL_DEBUG_STREAM("State "<<succ_state->state_id<< " has already been expanded in base decrease priority!");
                        m_open_base.decrease(succ_state,GroupType::BASE);
                    }
                    else {
                        m_open_base.push(succ_state,GroupType::BASE);
...
{
    SMPL_DEBUG("reorderOpen called!");
    
     m_open_base_iso.make(GroupType::BASE_ISO);

    for (auto it = m_open_base.begin(); it != m_open_base.end(); ++it) {
        (*it)->f[0] = computeKey(*it, GroupType::BASE);
    }
    m_open_base.make(GroupType::BASE);

    for (auto it = m_open_arm.begin(); it != m_open_arm.end(); ++it) {
        (*it)->f[1] = computeKey(*it, GroupType::ARM);
    }
    m_open_arm.make(GroupType::ARM);
}


int MARAStar::computeKey(SearchState* s,GroupType group)
{
...
void MARAStar::reinitSearchState(SearchState* state)
{
    if (state->call_number != m_call_number) {
        ...
        state->h[0] = m_heur->GetGoalHeuristic(state->state_id, GroupType::BASE);
        state->h[1] = m_heur->GetGoalHeuristic(state->state_id, GroupType::ARM);
